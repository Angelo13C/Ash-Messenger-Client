...
void DTPAReceiver::addToCachedRequests(DTPARequest request)
{
    ...
    if(!alreadyCachedRequest)
        _cachedRequests.push_back(request);

    //Call the function that needs to be answered
    DTPASender::getInstance()->callRequestAnswer(&request);

    bool deleteRequest = false;
    DTPAReceiverManager::getInstance()->callRequestCommand(&_cachedRequests[i], &deleteRequest);
